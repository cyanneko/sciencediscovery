...
async function run(command, args) {
  if (command === "pnpm" && args.includes("--recursive") && args.includes("test") && process.env.TEST_FAIL_FAST !== "1") args = ["--no-bail", ...args];
...
try {
  ...
  for (const [command, args] of layers[layer]) {
    ...
    const prerequisite = (command === "pnpm" && ["install", "build"].includes(args[0])) || (command === "uv" && args[0] === "sync");
    if (result.exitCode !== 0 && (prerequisite || process.env.TEST_FAIL_FAST === "1")) break;
  }
} catch (error) {
  exitCode ||= 2;
