...
- 展示回归重点：`ProcessLifecycle.test.tsx`、`Permissions.test.tsx`、`Orchestration.test.tsx`；浏览器覆盖 `process-timeline-presentation.spec.ts`、`subtask-delegation.spec.ts`、`plan-workspace.spec.ts`，检查运行/终态、独立子任务、Skill 生命周期、文件访问、计划状态与窄屏布局。
