...
现有自定义 MCP 测试入口：`services/api/src/mcp/custom-servers.test.ts`、`oauth.test.ts`、`node-client.test.ts`，`apps/web/tests/McpSecretFields.test.tsx`，以及 `test/e2e/mcp-server-management.spec.ts`、`mcp-oauth.spec.ts`、`mcp-secret-edit.spec.ts`。
