...
工具调用使用有界滚动执行池，不再通过无界 `Promise.all` 一次性派发。只有工具的
私有定义针对当前参数显式声明“并发安全”时才允许重叠执行；未声明或分类异常的工具
均作为独占排序屏障。`SCIENCE_AGENT_MAX_PARALLEL_TOOL_CALLS` 控制执行池宽度（默认
`10`，设为 `1` 时完全串行）。一个并发调用完成后会立即补入下一个排队调用，但结果
消息和完成事件仍按模型声明顺序提交。取消会停止补充队列，并等待已经启动的调用退出。
该配置限制的是瞬时并发压力，并不限制一次模型响应包含的工具调用总数。
...
```bash
./test/st/agent-runtime/run-agent-loop-mocked.sh        # 封闭：脚本化 SSE 模型端点 + 真实 Node 工具，验证原生 loop 全链路
./test/st/agent-runtime/run-agent-loop-real.sh      # 真实模型 + 真实工具
```
