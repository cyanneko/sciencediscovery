...
- `test/st/agent-runtime/run-agent-loop-mocked.sh` 与 `run-agent-loop-real.sh` 是现有适配器 smoke，直接在进程内构造 Agent，不因目录位置而成为 E2E。通过 `pnpm test:list` 和 `pnpm test --case <id>` 统一发现与执行。浏览器用例位于 `test/e2e/`，入口及分层规则见[测试指南](../../../test/README.md)。
