...
Existing custom MCP test entry points are `services/api/src/mcp/custom-servers.test.ts`, `oauth.test.ts`, `node-client.test.ts`, `apps/web/tests/McpSecretFields.test.tsx`, and `test/e2e/browser/journey-custom-mcp.spec.ts`, `journey-mcp-oauth.spec.ts`, `journey-mcp-secret-edit.spec.ts`.
