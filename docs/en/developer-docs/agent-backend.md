...
**Summarization.** One separate request goes to the same model endpoint (system prompt `You are compacting…`, no tools). `buildSummaryPrompt` receives the complete already-bounded source transcript, including tool-call ids, tool names, arguments, result previews, and stored refs; it does not apply a second 600/16,000-character clipping layer. Content is HTML-escaped inside `<new_messages>` / `<existing_summary>` — **escaping is a security requirement**. The requested scientific checkpoint separates completed work, evidence, decisions, terminal/abandoned leads, explicit pending requirements, optional leads, and one next step. Terminal failures must not silently become required work, while the model remains responsible for deciding whether to research further or synthesize.

**Checkpoint.** `summaryCheckpointMessage` builds a `role:"user"`, `name:"summary"` message whose body starts with `[ScienceDiscovery summary checkpoint]` inside `<durable_context_data>`, with `additional_kwargs` carrying `hide_from_ui: true` and `sciencediscovery_summary_checkpoint: true`. A lightweight validator merges duplicate sections, fills missing sections, reports multiple next actions and unknown tool-output refs, and checks the 6,000-character render budget. It does not classify scientific sufficiency. A non-shrinking checkpoint is rejected and retried once by default; attempts, validation warnings, and size estimates are exposed in context trace.
...
**Failure and recovery.** Stored tool bodies become head/tail preview + structured ref before summarization; the latest LLM step keeps its complete call/result relationship. A failed or non-shrinking summary keeps deterministic pruning and lets the run continue (unless cancelled). A Provider-classified context overflow triggers one forced reassembly and one retry of the same LLM turn; a second overflow is returned normally.
...
```bash
./test/st/agent-runtime/run-agent-loop-mocked.sh        # hermetic: scripted SSE model endpoint + real Node tools, full native loop
./test/st/agent-runtime/run-agent-loop-real.sh      # live model + real tools
```
