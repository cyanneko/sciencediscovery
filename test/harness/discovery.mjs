...
export function inventoryProblems(suites, directory = root, exclusions = []) {
  ...
  for (const file of walk(directory).filter(file => isTest(file) || /^test\/(?:e2e\/.*-journey\.mjs|st\/.*(?:smoke.*\.(?:ts|sh)|.*-test\.py)|contract\/cases\/.*\.json)$/.test(file))) if (!owners.has(file) && !exclusions.some(e => e.file === file)) errors.push(`Unowned test: ${file}`);
  return errors;
}
