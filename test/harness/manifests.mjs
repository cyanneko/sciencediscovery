...
export function normalizeCase(value, file, directory = root) {
  ...
  switch (value.runner) {
    case 'playwright': {
      ...
      if (source && (!source.startsWith('test/e2e/browser/') || !source.endsWith('.spec.ts'))) throw Error(`${file}: browser source outside spec directory`);
      command = ['bash','.ci/run-e2e.sh',group];
      if (source) env = { CI_E2E_SPEC: '(^|/)' + source.slice('test/e2e/browser/'.length).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$' };
      break;
    }
    ...
    case 'contract': {
      ...
      command = ['node','test/contract/run.mjs','--case',value.scenarioId,'--compare-env']; break;
    }
    default:
