...
test('public local and CI list wrappers produce identical selected IDs', () => {
  ...
  const local = cli('test/harness/cli.mjs', args), ci = cli('.ci/test-selector.mjs', args);
...
test('unknown selection fails before starting a process through either entry point', () => {
  for (const entry of ['test/harness/cli.mjs', '.ci/test-selector.mjs']) {
...
test('the actual Node entry point executes a deep test and propagates its failure', t => {
  ...
  const result = spawnSync(process.execPath, [join(root, 'test/harness/node-tests.mjs'), 'compiled'], { cwd: dir, encoding: 'utf8' });
