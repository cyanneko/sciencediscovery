 ...
 * Run: pnpm --filter @sciencediscovery/api... --filter @sciencediscovery/runner build && node test/st/api/runner-location.mjs
 */
...
try {
  ...
  if (process.argv.includes("--browser")) await step(`${steps.length + 1}. 浏览器验证相同位置交互与 SSH 兼容`, "共同目录、工作区、失败重试、窄屏、SSH/NPU 设置旅程均通过。", async () => {
    const child = spawn("npm", ["--prefix", ".e2e", "run", "test:mocked", "--", "runner-locations.spec.ts", "ssh-remote-runner.spec.ts", "npu-cards.spec.ts"], {
