...
import { validateModel, validateAssertions } from './evaluation.mjs';
import { root, walk, discover } from './discovery.mjs';
const EXECUTORS = ['native', 'jiuwenswarm'];
const CASE_KEYS = ['version','id','source','owner','description','layer','surface','runner','capabilities','llm','supportedExecutors','timeoutSeconds','requirements','gates','ci','tags','quarantine','scenarioId','command','resultPath','compatibility','reporting','assertions'];
...
export function normalizeCase(value, file, directory = root) {
  ...
  object(value.llm, ['mode','model'], `${file}: llm`);
  ...
  if (value.llm.mode === 'real') validateModel(value.llm.model, `${file}: llm.model`);
  else if (value.llm.model) throw Error(`${file}: only real subjects declare a model`);
  if (value.assertions.judge && (value.runner !== 'playwright' || !value.source)) throw Error(`${file}: judge assertions require a browser scenario`);
  if (value.assertions.judge && value.ci.pr) throw Error(`${file}: PR policy cannot call a real judge`);
  if (value.tags) {
    strings(value.tags, `${file}: tags`);
    if (value.tags.some(t => /^(llm|layer|assertions|external):/.test(t))) throw Error(`${file}: layer/llm/assertions/external tags are derived, not duplicated`);
  }
  ...
  return { ...value, source, configPath: file, executors: value.supportedExecutors, timeoutMs: value.timeoutSeconds * 1000,
    command, ...(group ? {group} : {}), ...(env ? {env}:{}), tags: [...(value.tags ?? []), `layer:${value.layer}`,`llm:${value.llm.mode}`,`assertions:${value.assertions.mode}`,`external:${value.llm.mode === 'real' || value.assertions.judge ? 'real' : 'none'}`] };
}
