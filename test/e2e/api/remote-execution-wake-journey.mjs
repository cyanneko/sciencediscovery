...
/**
 * E2E-META
 * Purpose: Finish a remote command, inspect its completion notice, and receive an assistant reply without replay.
 * Steps: Start API/local Runner plus a separate-store Runner; submit foreground/background jobs; inspect wake results and retained logs; continue the session.
 ...
 * Run: pnpm --filter @sciencediscovery/api... --filter @sciencediscovery/runner build && node test/e2e/api/remote-execution-wake-journey.mjs
 */
...
const root = resolve(".tmp", `remote-wake-${randomUUID()}`);
  ...
  try {
    ...
    if (wake) {
      ...
      if (!results.length) call = { name: "execution_status", arguments: { execution_id: notifications[0].sourceId } };
        ...
        assert.match(result.result.stdout, /remote-wake-complete/);
      }
      ...
      call = { name: "run_shell", arguments: { runner_id: runnerId,
        command: `${background ? "sleep 3; " : ""}printf 'once\\n' >> execution-count.txt; echo remote-wake-complete`,
...
try {
  ...
  await step("1. 启动存储隔离的产品服务", "API 与远程 Runner 使用不同 CAS，健康检查通过。", async () => {
    ...
    const host = await json("/api/remote-hosts", { alias: "separate-store", connectionKind: "direct",
  ...
  for (const [index, mode] of ["foreground", "background"].entries()) {
    await step(`${index + 2}. ${mode} 命令完成后继续会话`, "执行成功；wake 查询结果并回复；只提交一次命令；后续用户消息仍可完成。", async () => {
      const session = await json(`/api/projects/${project.id}/sessions`, { title: mode, modelId: model.id,
        remoteRunnerHostIds: [runnerId], approvalMode: "always_allow" });
      ...
      assert.match(output.chunks.map((chunk) => chunk.text).join(""), /remote-wake-complete/);
      ...
      return `Session ${session.id}; initial ${initial.id}, wake ${wake.id}, followup ${followup.id}: completed. Execution ${execution.id}: exit 0, one submission; Runner receipt exists remotely and is absent in API CAS.`;
    });
  }
  outcome = "PASS";
} catch (error) { console.error(redact(error.stack)); process.exitCode = 1; }
finally {
  await writeFile(resolve(root, "report.md"), `# Remote execution wake journey\n\nResult: ${outcome}\n\nTarget: ${sha}\n\nCommand: node test/e2e/api/remote-execution-wake-journey.mjs\n\nEnvironment: production API/Runner CLIs, separate remote CAS, loopback model stub; no SSH/NPU hardware or Web checks. Model calls: ${modelCalls.length}.\n\n${steps.map((item) => `## ${item.name}\n\nExpected: ${item.expected}\n\nActual: ${item.actual}\n\n${item.status}\n`).join("\n")}\n## Diagnostic log\n\n\`\`\`text\n${logs.slice(-5000)}\n\`\`\`\n`);
