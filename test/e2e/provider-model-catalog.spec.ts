...
test("J7 Provider 模型目录、失败降级与对话思考选择", { tag: "@mocked" }, async ({ journey, page }) => {
  ...
  // 已经存在的服务商——model-settings 迁移时得出的也是这个结论。
  // 这条旅程要的是「有服务商可看」，不是「从界面把它建出来」（那件事由
  // model-connect-wizard 覆盖），所以记录一律用 API 预置。
  /**
  ...
  try {
    ...
    // modelDiscovery——已迁进 model-connect-wizard 的「预设落地」一步。
    //
    ...
    await journey.step(
      ...
      async () => {
        const dialog = page.getByRole("dialog", { name: "系统设置" });
        // 从界面新建服务商的路径已并入连接卡片，由 model-connect-wizard
